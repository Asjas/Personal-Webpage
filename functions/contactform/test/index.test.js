...
it('should monitor Firestore', () => {
  ...
  assert.strictEqual(console.log.firstCall.args[0].includes(resourceId), true);
  assert.strictEqual(console.log.calledWith(JSON.stringify(data.oldValue, null, 2)), true);
  assert.strictEqual(console.log.calledWith(JSON.stringify(data.value, null, 2)), true);
});
...
it('should update data in response to Firestore events', () => {
  ...
  assert.strictEqual(console.log.calledWith('Replacing value: foobar --> FOOBAR'), true);
  assert.strictEqual(sample.mocks.firestore.set.calledWith({ original: 'FOOBAR' }), true);
});
...
it('should listen to Firebase Remote Config events', () => {
  ...
  assert.strictEqual(console.log.calledWith('Update type: INCREMENTAL_UPDATE'), true);
...
it('should make async HTTP request', () => {
  ...
  assert.strictEqual(sample.mocks.nodeFetch.calledWith('https://www.example.com'), true);
});
